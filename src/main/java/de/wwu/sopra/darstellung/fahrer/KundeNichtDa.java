...
public class KundeNichtDa extends Scene {
	...
	public KundeNichtDa(Stage primaryStage, double width, double height) {
		super(new BorderPane(), width, height);
		this.primaryStage = primaryStage;
		this.setRoot(root);
		root.setLeft(this.setGridPane());
		root.setCenter(new Label("this is Nicht Da"));
	}
	...
	private Button setBtKundeNichtDa() {
		if (btKundeNichtDa == null) {
			...
			btKundeNichtDa.setOnAction(e -> {
				// primaryStage.setScene(new KundeNichtDa(primaryStage, getWidth(),
				// getHeight()));
			});
		}
