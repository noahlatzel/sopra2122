...
/**
 * Die Klasse verwaltet den Zugriff der Kunden auf dessen Bestellungen, 
 * die Artikel, die persönlichen Daten, Rechnungen und den Warenkorb. 
 * Dies geschieht über die Grenzklassen PersoenlicheDatenAendern, 
 * PersoenlicheDatenAnzeigen, Suchen, Bestellen, Rechnung, 
 * WarenkorbAnsicht, BestellungVerwalten und BestellungenAnzeigen.  
 * 
 *
 */
public class Kundensteuerung {
	...
	public List<Produkt> suchen(String gesuchtesObjekt) throws NullPointerException {
		...
		if (gesuchtesObjekt == null) throw new NullPointerException();
	...
	public void bestellen() {
		...
		Bestellung bestellung = new Bestellung(0,LocalDateTime.now(),
				kunde.getWarenkorb().getProdukte(),kunde);
	...
	public void nachbestellen(Bestellung bestellung) throws NullPointerException {
		...
		Bestellung nachbestellung = new Bestellung(0,LocalDateTime.now(),
				neueProduktListe(bestellung.getProdukte()),kunde);
		kunde.bestellungHinzufuegen(nachbestellung);
	}
	
	/**
	 * Methode zum kopieren einer Liste mit gleichen Produkten aber neuen Produkt-Objekten
